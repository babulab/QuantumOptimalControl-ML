...
class RLQuantumOptimiserPPO:
    def __init__(self, circuit_fn, n_thetas, initial_state, target_state, num_qubits, hyperparams):
        self.env = DummyVecEnv([lambda: QuantumEnv(circuit_fn, n_thetas, initial_state, target_state, num_qubits)])
    ...
    def optimise(self):
        ...
        self.info = {'states_clipped':self.env.envs[0].states,
                    'best_fidelity':-self.env.envs[0].best_value,
